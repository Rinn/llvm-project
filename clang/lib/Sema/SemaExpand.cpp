...
TryBuildIterableExpansionStmtInitializer(Sema &S, Expr *ExpansionInitializer,
  ...
  DeclSpec DS{AF};
  {
    ...
    unsigned DiagID;
    const PrintingPolicy &Policy = S.getASTContext().getPrintingPolicy();
    DS.setTypeArgumentRange(BeginMinusBegin.get()->getSourceRange());
    if (DS.SetTypeSpecType(DeclSpec::TST_decltype, ColonLoc, PrevSpec, DiagID,
                           BeginMinusBegin.get(), Policy)) {
      S.Diag(ColonLoc, DiagID) << PrevSpec;
      return Data;
    }
  ...
  ParsedType TypeRep = S.ActOnTypeName(DeclaratorInfo).get();
  ExprResult AddRHS = S.ActOnCXXTypeConstructExpr(
      TypeRep, Index->getBeginLoc(), MultiExprArg(&Index, 1),
      Index->getEndLoc(), /*ListInitialization=*/true);
  if (AddRHS.isInvalid())
    return Data;
...
StmtResult Sema::BuildNonEnumeratingCXXExpansionStmtPattern(
  ...
  if (Data.isIterable()) {
    ...
    if (FinaliseExpansionVar(*this, ExpansionVar, Deref.get()))
      return StmtError();
